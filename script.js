class SnakeRace{
	...
	iniciarReconocimientoVoz = () => {
		...
		reconocimiento.lang = "es-ES"; // Configuración en español
		reconocimiento.continuous = true;
		reconocimiento.interimResults = false;
	
		reconocimiento.onresult = (evento) => {
			const transcripcion = evento.results[evento.results.length - 1][0].transcript.trim().toLowerCase();
			
			switch (transcripcion) {
				case "arriba.":
					if (this.cabeza.dir !== 2) this.cabeza.giro = 0;
					break;
				case "derecha.":
					if (this.cabeza.dir !== 3) this.cabeza.giro = 1;
					break;
				case "abajo.":
					if (this.cabeza.dir !== 0) this.cabeza.giro = 2;
					break;
				case "izquierda.":
					if (this.cabeza.dir !== 1) this.cabeza.giro = 3;
					break;
				default:
					console.log(`Comando no reconocido: ${transcripcion}`);
			}
		};
	
		reconocimiento.onerror = (error) => console.error("Error en reconocimiento de voz:", error);
...
switch (tipo) {
    case 0,1: 
        ...
        divTexto.textContent = 'Pilotar es ir siempre al límite. Lo difícil es saber dónde están tus límites, los de tu coche y los de la pista. Y luego, ampliarlos.';
        textToSpeech(divTexto.textContent); // Convierte el texto a voz
        break;
    case 2: // Abandono
